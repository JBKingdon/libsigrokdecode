...
# sx1262 status combines the current mode and the status of the last command

#       Chip mode         Command status
# 0x0:  Unused              Reserved
# 0x1:  RFU                 RFU
# 0x2:  STBY_RC             Data is available to host
# 0x3: STBY_XOSC            Command timeout
# 0x4: FS                   Command processing error
# 0x5: RX                   Failure to execute command
# 0x6: TX                   Command TX done
...
sx1262_status_status = {
    1 : 'OK',           # not actually documented, but seems to be used as such
    2 : 'DATA AVAIL',
    3 : 'CMD TIMEOUT',
    4 : 'CMD P ERROR',
    5 : 'CMD EX FAIL',
...
CMD_SET_RXTX_FALLBACK_MODE = 0x93
...
sx1262_commands = {
    ...
    CMD_CLEAR_DEVICE_ERRORS:    ('CLEAR DEVICE ERRS',    2),
    CMD_GET_DEVICE_ERRORS:      ('GET DEVICE ERRS',      4),
    ...
    CMD_SET_PACKETPARAMS:       ('SET PKT PRMS',         6),
    CMD_CALIBRATE:              ('CALIBRATE',            1),
    CMD_SET_DIOIRQPARAMS:       ('SET DIO IRQ',          1),
    CMD_SET_TXPARAMS:           ('SET TX PRMS',          2),
    CMD_SET_BUFFERBASEADDRESS:  ('SET BUFFER BASE ADDR', 1),
    CMD_SET_RXTX_FALLBACK_MODE: ('SET RXTX FALLBACK MODE',1),
    CMD_CLEAR_IRQ_STATUS:       ('CLR IRQSTATUS',        2),
    CMD_SET_PA_CONFIG:          ('SET PA CONFIG',        4),
    CMD_SET_REGULATORMODE:      ('SET REGULATOR MODE',   1),
    CMD_SET_DIO3_AS_TXCO_CTRL:  ('SET DIO3 TXCO',        4),
    CMD_CALIBRATE_IMAGE:        ('CALIBRATE IMAGE',      2),
...
class Decoder(srd.Decoder):
    ...
    id = 'sx1262'
    name = 'sx1262'
    longname = 'Semtech sx1262'
    desc = '915MHz Lora Radio'
    license = 'gplv2+'
    inputs = ['spi']
    outputs = []
    tags = ['IC', 'Wireless/RF']
    options = (
        {'id': 'chip', 'desc': 'Chip type',
            'default': 'sx1262', 'values': ('sx1262', 'somethingElse')},
    )
    annotations = (
    ...
    def parse_command(self, b):
        ...
        if b in sx1262_commands:
            commandName   = sx1262_commands[b][0]
            commandLength = sx1262_commands[b][1]
    ...
    def decode_register(self, pos, ann, regid, data):
        ...
        if name == 'STATUS':
            ...
            statusMode = (statusByte & 0b1110000) >> 4
            statusCmdStatus = (statusByte &0b0001110) >> 1
            # TODO convert numbers to text descriptions
            if statusMode in sx1262_status_modes:
                smText = sx1262_status_modes[statusMode]
            else:
                smText = '{}?'.format(statusMode)

            if statusCmdStatus in sx1262_status_status:
                sStatusText = sx1262_status_status[statusCmdStatus]
            else:
